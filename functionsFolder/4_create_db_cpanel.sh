function create_db_on_cpanel() {
    ...
    for DB in $DBS; do
        ...
        if [ $? -eq 0 ]; then
            ...
            log "Error creating "$USER_CPANEL"_"$DB" database"
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            log "Error creating "$USER_CPANEL"_"$DB_USER" "
        fi
                ...
                \"blocks\": [
                    {
                        ...
                        \"text\": {
                            \"type\": \"plain_text\",
                            \"text\": \"Upload DATABASES dumps for $USER_CPANEL COMPLETED - $WORKER\",
                            \"emoji\": true
                        }
                    },
                    {
